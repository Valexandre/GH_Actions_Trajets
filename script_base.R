...
if (jour %in% c(1, 3, 5, 7)) {
  ...
  ComSelec <- sample(JusteCommunesImportantes$INSEE_COM[substr(JusteCommunesImportantes$INSEE_COM, 1, 2) %in% c(75, 77, 78, 91:95) &
    JusteCommunesImportantes$INSEE_COM %!in% dejaparus$codeinsee[dejaparus$categorietweet == "Origine et provenance des actifs"]], 1)
  ...
  dejaparus <- read_csv("data/dejaparus.csv", col_types = cols(codeinsee = col_character(), datetweets = col_character()))
  ComSelec <- sample(JusteCommunesImportantes$INSEE_COM[substr(JusteCommunesImportantes$INSEE_COM, 1, 2) %in% c(75, 77, 78, 91:95) & JusteCommunesImportantes$INSEE_COM %!in% dejaparus$codeinsee[dejaparus$categorietweet == "Trajets auto évitables"]], 1)
